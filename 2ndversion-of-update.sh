...
image="hellolove123/cab-booking-app::xyz"

# Remove the tag from the image name
image=${image%:*}

# Replace the image name in the deployment.yaml file
sed -i "s|hellolove123/cab-booking-app.*|$image|g" deployment.yaml
...
latest_tag=$(docker images | grep hellolove123/cab-booking-app | awk '{print $2}' | head -n 1)

# Update the deployment file with the latest tag
sed -i "s|hellolove123/cab-booking-app:.*|hellolove123/cab-booking-app:$latest_tag|g" deployment.yaml
